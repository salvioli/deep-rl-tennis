...
class DDPGAgent:
    ...
    def __init__(self, state_size, action_size, random_seed,
        ...
        self.actor_local = Actor(state_size, action_size, self.seed, self.actor_nn_size[0], self.actor_nn_size[1], self.batch_norm).to(device)
        self.actor_target = Actor(state_size, action_size, self.seed, self.actor_nn_size[0], self.actor_nn_size[1], self.batch_norm).to(device)
        self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=self.lr_actor)

        # Critic Network (w/ Target Network)
        self.critic_local = Critic(state_size, action_size, self.seed, self.critic_nn_size[0], self.critic_nn_size[1], self.batch_norm).to(device)
        self.critic_target = Critic(state_size, action_size, self.seed, self.critic_nn_size[0], self.critic_nn_size[1], self.batch_norm).to(device)
        self.critic_optimizer = optim.Adam(self.critic_local.parameters(), lr=self.lr_critic,
                                           weight_decay=self.weight_decay)

        # Noise process
        self.noise = OUNoise(action_size, self.seed, sigma=self.sigma)
    ...
    def copy_weights_from(self, agent):
        self.actor_target.load_state_dict(agent.actor_target.state_dict())
        self.actor_local.load_state_dict(agent.actor_local.state_dict())
        self.critic_target.load_state_dict(agent.critic_target.state_dict())
        self.critic_local.load_state_dict(agent.critic_local.state_dict())

    def clone(self):
        new_obj = type(self)(self.state_size, self.action_size, self.seed)
        new_obj.__dict__.update(self.__dict__)

        new_obj.copy_weights_from(self)
